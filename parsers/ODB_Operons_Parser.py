...
#############################################################################
# all unnecessary references to operon have been removed at this point.     #
# unnecessary outputs have been removed. an option for quiet might be nice  #
#############################################################################
...
def regulon_db(outfolder, number_of_genes, url, download, experimantal, outfile):
    url_outfile = outfolder +"OperonSet.txt"
    if download:
        download_remote_file(url, url_outfile)
        ...
        if feature.type == 'CDS' or feature.type == 'ncRNA' or feature.type == 'tRNA' or feature.type == 'mRNA' or feature.type == 'rRNA':
            ...
            try:
                ...
                try:
                    ...
                    locus = ''
                    print "No locus associated with a reporrted feature in accession %s This should never be invoked meaning the gbk file has an error." % accession
            try: 
